...
export class SoulboundTokenController {
  ...
  @ApiOperation({ 
    ...
    description: 'Manually issue a badge/achievement to a user (admin or system use)'
  })
  @ApiResponse({
    status: 201,
  ...
  issueToken(
    ...
    return this.soulboundTokenService.issueToken(userId, dto.name, dto.metadata);
  }

  @Post('check-badges')
  @ApiOperation({ 
    summary: 'Check and auto-issue badges',
    description: 'Automatically check user achievements and issue eligible badges'
  })
  @ApiResponse({
    status: 200,
