...
export class SoulboundTokenService {
  ...
  async checkAndIssueBadges(userId: string) {
    ...
    if (
      user.lands.some((l) => l.plant) &&
      !existingBadgeNames.has(this.BADGE_CONFIGS.FIRST_PLANT.name)
    ) {
      try {
    ...
    if (
      harvestCount >= 10 &&
      !existingBadgeNames.has(this.BADGE_CONFIGS.HARVEST_MASTER.name)
    ) {
      try {
    ...
    if (
      ...
      !existingBadgeNames.has(this.BADGE_CONFIGS.MISSION_COMPLETER.name)
    ) {
      try {
    ...
    if (
      hasGoldBranch &&
      !existingBadgeNames.has(this.BADGE_CONFIGS.GOLD_BRANCH_OWNER.name)
    ) {
      try {
    ...
    if (issuedBadges.length > 0) {
      this.logger.log(
        `Auto-issued ${issuedBadges.length} badges to user ${userId}: ${issuedBadges.join(", ")}`,
      );
    }
  ...
  private groupByCategory(tokens: any[]) {
    ...
    tokens.forEach((token) => {
      const category = token.metadata?.category || "other";
      if (!grouped[category]) {
        grouped[category] = [];
      }
