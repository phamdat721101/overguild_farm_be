...
export class FertilizerController {
  ...
  @ApiOperation({
    ...
    description:
      "Apply fertilizer to boost plant growth and improve soil quality",
  })
  @ApiResponse({
    status: 200,
  ...
  applyFertilizer(
    ...
    return this.fertilizerService.applyFertilizer(
      userId,
      dto.landId,
      dto.fertilizerType,
    );
  ...
  @ApiOperation({
    ...
    description:
      "Burn fruits to convert them into fertilizer. Rates: 3 fruits = 1 Common, 10 = 1 Rare, 30 = 1 Epic, 100 = 1 Legendary",
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      example: {
        success: true,
        fruitsConsumed: 30,
        fertilizersCreated: [{ type: "FERTILIZER_EPIC", amount: 1 }],
  ...
  compostFruits(@CurrentUser("sub") userId: string, @Body() dto: CompostDto) {
